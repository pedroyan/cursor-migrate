import fs from "node:fs";
import path from "node:path";
import { DatabaseSync } from "node:sqlite";
import { replaceExactPath } from "./path-replace.js";
import { globalStorageDbPath, toFileUri } from "./paths.js";

export function countComposerHeadersForWorkspace(workspaceId) {
  const dbPath = globalStorageDbPath();
  if (!fs.existsSync(dbPath)) {
    return 0;
  }
  const db = new DatabaseSync(dbPath, { readOnly: true });
  try {
    const table = db
      .prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'composerHeaders'")
      .get();
    if (table) {
      const total = db.prepare("SELECT COUNT(*) AS n FROM composerHeaders").get()?.n ?? 0;
      if (total > 0) {
        return (
          db.prepare("SELECT COUNT(*) AS n FROM composerHeaders WHERE workspaceId = ?").get(workspaceId)?.n ?? 0
        );
      }
    }
    const row = db.prepare("SELECT value FROM ItemTable WHERE key = 'composer.composerHeaders'").get();
    if (!row?.value) {
      return 0;
    }
    const data = JSON.parse(row.value);
    return (data.allComposers ?? []).filter((c) => c.workspaceIdentifier?.id === workspaceId).length;
  } finally {
    db.close();
  }
}

export function patchWorkspaceDbPaths(workspaceDir, fromPath, toPath) {
  const dbPath = path.join(workspaceDir, "state.vscdb");
  if (!fs.existsSync(dbPath)) {
    return 0;
  }

  const db = new DatabaseSync(dbPath);
  const rows = db.prepare("SELECT key, value FROM ItemTable").all();
  const update = db.prepare("UPDATE ItemTable SET value = ? WHERE key = ?");
  let changed = 0;
  const fromUri = toFileUri(fromPath);
  const toUri = toFileUri(toPath);

  for (const row of rows) {
    if (typeof row.value !== "string") {
      continue;
    }
    let next = replaceExactPath(row.value, fromPath, toPath);
    next = replaceExactPath(next, fromUri, toUri);
    if (next !== row.value) {
      update.run(next, row.key);
      changed += 1;
    }
  }

  db.close();
  return changed;
}
