...
import { replaceMigrationPaths } from "./path-replace.js";
...
function patchWorkspaceObject(obj, m) {
  ...
  const oldName = path.basename(m.fromPath);
  const newName = path.basename(m.toPath);
  if (
    obj.name === oldName &&
    oldName &&
    oldName !== newName &&
    (obj.type === "workspace" || typeof obj.displayPath === "string" || typeof obj.folderUri === "string")
  ) {
    obj.name = newName;
    changed = true;
  }
...
function replaceStrings(value, m) {
  return replaceMigrationPaths(value, m);
}
...
function patchStorageJson(migration, file = storageJsonPath()) {
  ...
  const next = replaceMigrationPaths(text, migration);
  if (next !== text) {
