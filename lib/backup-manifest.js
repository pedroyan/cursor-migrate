...
export function workspaceStorageArtifactName(workspaceId) {
  return `workspaceStorage-${workspaceId}`;
}

/** Workspace hash folders repair may read or overwrite — back all of them up before repair runs. */
export function collectRepairWorkspaceIds({
  originMatches,
  destinationMatches,
  activeWorkspaceId,
  primaryId,
...
export function appendWorkspaceStorageBackups({
  artifacts,
  backupDir,
  workspaceIds,
  workspaceRoot,
  ...
  for (const id of workspaceIds) {
    const artifact = workspaceStorageArtifactName(id);
    if (artifacts.includes(artifact)) {
      continue;
    }
    if (backupIfExists(path.join(workspaceRoot, id), backupDir, artifact)) {
