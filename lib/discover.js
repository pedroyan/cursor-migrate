...
const DISTINCT_ACTIVITY_MS = 5_000;

function activityTime(match) {
  return Math.max(match.mtime ?? 0, match.walMtime ?? 0);
}
...
export function findWorkspaceStorageIdsForFolder(absPath, fileUri, { workspaceRoot = workspaceStorageRoot() } = {}) {
  ...
  for (const entry of fs.readdirSync(workspaceRoot, { withFileTypes: true })) {
    ...
    const wal = path.join(dir, "state.vscdb-wal");
    const size = fs.existsSync(db) ? fs.statSync(db).size : 0;
    const mtime = fs.existsSync(db) ? fs.statSync(db).mtimeMs : 0;
    const walMtime = fs.existsSync(wal) ? fs.statSync(wal).mtimeMs : 0;
    matches.push({ id: entry.name, size, mtime, walMtime, folder: ws.folder });
  }
...
export function findActiveWorkspaceStorageId(absPath, fileUri, options) {
  ...
  const byActivity = [...matches].sort((a, b) => activityTime(b) - activityTime(a));
  const newest = byActivity[0];
  const second = byActivity[1];
  if (second && activityTime(newest) - activityTime(second) >= DISTINCT_ACTIVITY_MS) {
    return newest.id;
  }

  try {
    const predicted = pickWorkspaceId(absPath);
    if (matches.some((match) => match.id === predicted)) {
      return predicted;
    }
  } catch {
    // Destination folder may be missing during tests that only stub workspaceStorage.
  }
