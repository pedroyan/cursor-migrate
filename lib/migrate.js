...
async function repairProject(options) {
  ...
  const originIds = originMatches.map((m) => m.id).filter(Boolean);
  const composersBefore = dryRun ? 0 : countComposersOnWorkspaceIds(originIds);
  ...
  if (!dryRun) {
    ...
    if (composersBefore > 0 && mapped === 0) {
      throw new Error(
        `composerHeaders was not remapped (0 conversations on ${wsResult.newWorkspaceId}, ${composersBefore} still on origin workspace id(s)). ` +
    ...
    if (globalResult) {
      log(
        "global-storage",
        `Updated ${globalResult.updatedRows} rows; ${mapped} conversations on ${wsResult.newWorkspaceId}`,
      );
    }
  } else {
...
function finishGlobalPatch(ctx) {
  ...
  if (dryRun) {
    log(
      "global-storage",
      `Would patch composerHeaders from ${originIds.length} origin workspace id(s)`,
    );
    return { fromPath, toPath, backupDir, dryRun, projectMeta, wsResult, done: true, repair };
  }
