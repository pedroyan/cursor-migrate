...
function moveProjectDirectory(fromPath, toPath) {
  try {
...
export async function migrateProject(options) {
  ...
  if (moveRepo && fs.existsSync(toPath)) {
    throw new Error(
      `Destination already exists: ${toPath} — --to must be the full destination path including the project folder name`,
    );
  }
  if (!moveRepo && !fs.existsSync(toPath)) {
  ...
  if (moveRepo) {
    ...
    if (!dryRun) {
      crossVolumeMove = moveProjectDirectory(fromPath, toPath);
    }
  }
